...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', '--src', default='./data/cropped.v2', help='directory of raw images')
    parser.add_argument('-d', '--dst', default='./data/aligned.v2', help='directory of aligned images')
    parser.add_argument('-o', '--output_size', default=1024, type=int, help='size of aligned output (default: 256)')
    ...
    movies = ['theincredibles','bossbaby']
    for movie in movies:
        src_dir = os.path.join(args.src,movie)  
        dst_dir = os.path.join(args.dst,movie)
        if not os.path.exists(dst_dir):
            os.mkdir(dst_dir)  
        for img_name in os.listdir(src_dir):
            raw_img_path = os.path.join(src_dir, img_name)
            # import pdb;pdb.set_trace()
            # get_landmarks accepts both img or path -> directly send img
            img = cv2.imread(raw_img_path)
            # import pdb;pdb.set_trace()
            try:
                for i, face_landmarks in enumerate(landmarks_detector.get_landmarks(img), start=1):

                    aligned_face_path = os.path.join(dst_dir, f'aligned_{img_name}')

                    image_align(raw_img_path, aligned_face_path, face_landmarks, args.output_size, args.transform_size, args.no_padding)
            except Exception as e:
                print(e)
                print('not working for %s'%raw_img_path)